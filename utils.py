...
class ExpTargetIterScheduler(_LRScheduler):
    def __init__(self, optimizer, gamma, target_iteration, min_lr=1e-7, last_epoch=-1, verbose=False):
        self.gamma = gamma
...
def calc_iou(truth_mask, pred_mask, class_list, pixel_limit, smooth=1e-5):
    iou_list = dict()
    # IOU
    for cls_idx in class_list:
        ...
        if torch.sum(truth_mask==cls_idx)>pixel_limit:
            ...
            cls_pred_mask = pred_mask==cls_idx

            cls_and = (torch.sum(torch.logical_and(cls_truth_mask, cls_pred_mask))).item()
            cls_or = (torch.sum(torch.logical_or(cls_truth_mask, cls_pred_mask))).item()
...
def calc_dice(truth_mask, pred_mask, class_list, pixel_limit, smooth=1e-5):
    dice_list = dict()
    # DICE
    for cls_idx in class_list:
        ...
        if torch.sum(truth_mask==cls_idx)>pixel_limit:
            ...
            cls_and = (torch.sum(torch.logical_and(cls_truth_mask, cls_pred_mask))).item()
            cls_x_y = torch.sum(cls_truth_mask).item() + torch.sum(cls_pred_mask).item()
